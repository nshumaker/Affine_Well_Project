...
def plot_meshgrid(meshgrid_args, plotting_args):
    ...
    hist_x_min = np.min(Z_filt, axis=0)
    hist_y_min = np.min(Z_filt, axis=1)
    ...
    cs = ax.contourf(xx, yy, Z_filt, 
                     ...
                     **plotting_args)
    #plot marker
    mrker = ax.plot(val_x, val_y, '+', markeredgecolor='k', markeredgewidth=3, ms=16)
    #plot text
    txt = ax.annotate('    P90 {}\n    Mean {}\n    P10 {}'.format(dict_y_hat['P90'],dict_y_hat['mean'],dict_y_hat['P10']),
                      xy=(val_x, val_y), xycoords='data', xytext=(val_x,val_y), va='center', weight='heavy', color='k')
    ...
    ax_hist_x.fill_between(xx[0,:], hist_x_max, hist_x_min, facecolor='darkgray', alpha=.8, edgecolor='k',linewidth=2)
    ax_hist_y.fill_betweenx(yy[:,0], hist_y_max, hist_y_min, facecolor='darkgray', alpha=.8, edgecolor='k',linewidth=2)
    
    ax_hist_x.set_xlim([xx.min(), xx.max()])
    ax_hist_x.set_ylim([plotting_args['vmin'], plotting_args['vmax']])
    
    
    ax_hist_y.set_xlim([plotting_args['vmin'], plotting_args['vmax']])
    ax_hist_y.set_ylim([yy.min(), yy.max()])
    
    ax_hist_x.set_xticks([])
    ax_hist_x.yaxis.set_ticks_position('right')
    ax_hist_x.tick_params(axis="y",direction="in", pad=-22)
    ax_hist_x.set_ylabel('Gas (mcf/ft)', weight='heavy')
    ...
    ax_hist_y.set_xlabel('Gas (mcf/ft)', weight='heavy')
    
    #colorbar
    bar = fig.colorbar(cs, ax=ax_hist_y)
    bar.set_label('Gas Production (mcf/ft)', weight='heavy', size=12) 
...
dict_plotting_args1 = {'cmap':plt.cm.magma,'vmin':0,'vmax':175,'aspect':'auto',
                     'origin':'lower'}
dict_plotting_args1a = {'cmap':plt.cm.YlGn,'vmin':60,'vmax':180}

dict_test_args = {'vmin':60,'vmax':180,'cbsteps':13, 'cmap':plt.cm.YlGn}

dict_plotting_args2 = {'cmap':plt.cm.magma_r,'vmin':3,'vmax':7,'aspect':'auto',
                     'origin':'lower'}

hist_x = plot_meshgrid(dict_kwargs1, dict_test_args)
